...
RegisterNetEvent('esx_vagosjob:objectspawner_cl')
AddEventHandler('esx_vagosjob:objectspawner_cl', function(model, x, y, z, h)
  ESX.Game.SpawnObject(model, {
    x = x,
    y = y,
    z = z
...
function OpenVehicleInfosServiceMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_vagosjob:getVehicleInfosService', function(infos)
...
----TriggerEvent('es:RegisterCommand', 'shield', 'vagos', function(source, args, user)
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'vagos' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if
              GetEntityModel(vehicle) == GetHashKey('vagos')  or
              GetEntityModel(vehicle) == GetHashKey('vagos2') or
              GetEntityModel(vehicle) == GetHashKey('vagos3') or
              GetEntityModel(vehicle) == GetHashKey('vagosb') or
              GetEntityModel(vehicle) == GetHashKey('pbus') or
              GetEntityModel(vehicle) == GetHashKey('riot') or
			  GetEntityModel(vehicle) == GetHashKey('vagost')
            then
    ...
    if IsControlPressed(0,  Keys['G']) and GetLastInputMethod( 0 ) and PlayerData.job ~= nil and PlayerData.job.name == 'vagos' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'vagos_actions') and (GetGameTimer() - GUI.Time) > 150 then
