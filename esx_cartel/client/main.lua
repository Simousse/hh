...
function OpenCloakroomMenu()
  ...
  ESX.UI.Menu.Open(
    ...
    function(data, menu)
      ...
      CurrentAction     = 'cartel_actions_menu'
...
RegisterNetEvent('esx_carteljob:objectspawner_cl')
AddEventHandler('esx_carteljob:objectspawner_cl', function(model, x, y, z, h)
  ESX.Game.SpawnObject(model, {
    x = x,
    y = y,
    z = z
...
function OpenVehicleInfosServiceMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_carteljob:getVehicleInfosService', function(infos)
...
----TriggerEvent('es:RegisterCommand', 'shield', 'cartel', function(source, args, user)
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if
              GetEntityModel(vehicle) == GetHashKey('cartel')  or
              GetEntityModel(vehicle) == GetHashKey('cartel2') or
              GetEntityModel(vehicle) == GetHashKey('cartel3') or
              GetEntityModel(vehicle) == GetHashKey('cartelb') or
              GetEntityModel(vehicle) == GetHashKey('pbus') or
              GetEntityModel(vehicle) == GetHashKey('riot') or
			  GetEntityModel(vehicle) == GetHashKey('cartelt')
            then
    ...
    if IsControlPressed(0,  Keys['G']) and GetLastInputMethod( 0 ) and PlayerData.job ~= nil and PlayerData.job.name == 'cartel' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'cartel_actions') and (GetGameTimer() - GUI.Time) > 150 then
