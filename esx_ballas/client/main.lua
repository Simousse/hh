...
RegisterNetEvent('esx_ballasjob:objectspawner_cl')
AddEventHandler('esx_ballasjob:objectspawner_cl', function(model, x, y, z, h)
  ESX.Game.SpawnObject(model, {
    x = x,
    y = y,
    z = z
...
function OpenVehicleInfosServiceMenu(vehicleData)
  ...
  ESX.TriggerServerCallback('esx_ballasjob:getVehicleInfosService', function(infos)
...
----TriggerEvent('es:RegisterCommand', 'shield', 'ballas', function(source, args, user)
...
Citizen.CreateThread(function()
  while true do
      ...
      if IsControlPressed(0,  Keys['E']) and PlayerData.job ~= nil and PlayerData.job.name == 'ballas' and (GetGameTimer() - GUI.Time) > 150 then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if
              GetEntityModel(vehicle) == GetHashKey('ballas')  or
              GetEntityModel(vehicle) == GetHashKey('ballas2') or
              GetEntityModel(vehicle) == GetHashKey('ballas3') or
              GetEntityModel(vehicle) == GetHashKey('ballasb') or
              GetEntityModel(vehicle) == GetHashKey('pbus') or
              GetEntityModel(vehicle) == GetHashKey('riot') or
			  GetEntityModel(vehicle) == GetHashKey('ballast')
            then
    ...
    if IsControlPressed(0,  Keys['G']) and GetLastInputMethod( 0 ) and PlayerData.job ~= nil and PlayerData.job.name == 'ballas' and not ESX.UI.Menu.IsOpen('default', GetCurrentResourceName(), 'ballas_actions') and (GetGameTimer() - GUI.Time) > 150 then
